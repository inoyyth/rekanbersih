<form method="post" action="<?=base_url();?>menu_bottom/update_proses" enctype="multipart/form-data" >
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="menu_parent" required>
                ...
                <option style="color: #000;" value="<?="1-".$menux->id;?>" <?=$cek1;?>><?=$menux->menu_name;?></option>
                    ...
                    $idx2=$list->id;
                    //echo $mid."<br>".$idx;
                    $sq=mysql_query("select * from menu_bottom where menu_parent_id='$mid' and id not in ($idx2) order by menu_position asc");
                    ...
                    <option style="color: #0063DC;" value="<?="2-".$daty['id'];?>" <?=$cek2;?>> - <?=$daty['menu_name'];?></option>
                        <?php
                        $sqx=mysql_query("select * from menu_bottom where menu_parent_id='".$daty['id']."' and id not in ($idx2) order by menu_position asc");
                        ...
                        <option style="color: #86d5f8;" value="<?="3-".$datx['id'];?>" <?=$cek3;?>>  &nbsp;&nbsp; - - <?=$datx['menu_name'];?></option>
                            <?php
                            $sqc=mysql_query("select * from menu_bottom where menu_parent_id='".$datx['id']."' and id not in ($idx2) order by menu_position asc");
                            ...
                            <option style="color: #800;" value="<?="4-".$datc['id'];?>" <?=$cek4;?>>  &nbsp;&nbsp; - - - <?=$datc['menu_name'];?></option>
                                <?php
                                $sqb=mysql_query("select * from menu_bottom where menu_parent_id='".$datc['id']."' and id not in ($idx2) order by menu_position asc");
                                ...
                                <option style="color: #f0ad4e;" value="<?="5-".$datb['id'];?>" <?=$cek5;?>>  &nbsp;&nbsp; - - - - <?=$datb['menu_name'];?></option>
                                    <?php
                                    $sqd=mysql_query("select * from menu_bottom where menu_parent_id='".$datb['id']."' and id not in ($idx2) order by menu_position asc");
