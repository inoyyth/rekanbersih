<form method="post" action="<?=base_url();?>menu_bottom/add_proses" enctype="multipart/form-data" >
<div style="margin-bottom: 5px;text-align: right;">
    <input type="submit" class="btn btn-primary" value="Save" /> <input type="button" onclick="window.location.replace('<?php echo base_url(); ?>menu_bottom');" class="btn btn-danger" value="Cancel" />
</div>
<div class="panel panel-success">
    ...
    <div class="panel-body" id="panelx">
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="menu_parent" required>
                ...
                <option value="<?="1-".$menux->id;?>"><?=$menux->menu_name;?></option>
                    <?php
                    $mid=$menux->id;
                    $sq=mysql_query("select * from menu_bottom where menu_parent_id='$mid' order by menu_position asc");
                    while($daty=  mysql_fetch_array($sq)){?>
                    <option value="<?="2-".$daty['id'];?>"> - <?=$daty['menu_name'];?></option>
                        <?php
                        $sqx=mysql_query("select * from menu_bottom where menu_parent_id='".$daty['id']."' order by menu_position asc");
                        while($datx=  mysql_fetch_array($sqx)){?>
                        <option value="<?="3-".$datx['id'];?>">  &nbsp;&nbsp; - - <?=$datx['menu_name'];?></option>
                            <?php
                            $sqc=mysql_query("select * from menu_bottom where menu_parent_id='".$datx['id']."' order by menu_position asc");
                            while($datc=  mysql_fetch_array($sqc)){?>
                            <option value="<?="4-".$datc['id'];?>">  &nbsp;&nbsp; - - - <?=$datc['menu_name'];?></option>
                                <?php
                                $sqb=mysql_query("select * from menu_bottom where menu_parent_id='".$datc['id']."' order by menu_position asc");
                                while($datb=  mysql_fetch_array($sqb)){?>
                                <option value="<?="5-".$datb['id'];?>">  &nbsp;&nbsp; - - - - <?=$datb['menu_name'];?></option>
                                    <?php
                                    $sqd=mysql_query("select * from menu_bottom where menu_parent_id='".$datb['id']."' order by menu_position asc");
                                    while($datd=  mysql_fetch_array($sqd)){?>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="menu_value" name="menu_value" autocomplete="off"  required/><a href="#" id="linkx">Search</a>
