...
class Menu_bottom extends MX_Controller{
    ...
    function index(){
        $data['data']=$this->db->query("select a.*,b.menu_type as menu_typex from menu_bottom a left join menu_type b on a.menu_type=b.id where a.menu_index='0' order by a.menu_position asc")->result();
        $data['max']=$this->db->query("select max(menu_position) as max_pos from menu_bottom where menu_index='0'")->row();
        $data['min']=$this->db->query("select min(menu_position) as min_pos from menu_bottom where menu_index='0'")->row();
        $data['view']='main';
    ...
    function add(){
        ...
        $data['menu']=$this->db->query("select * from menu_bottom where menu_parent_id='0' order by id")->result();
        $data['view']='add';
        $this->load->view('template',$data);
    }
    ...
    function add_proses(){
        ...
        $position="";
        if($menu_parent=="0"){
            $pos=$this->db->query("select max(menu_position) as maxid from menu_bottom where menu_parent_id='0'")->row('maxid');
            $position=$pos + 1;
        }else{
            $pos=$this->db->query("select max(menu_position) as maxid from menu_bottom where menu_parent_id='$menu_parentx'")->row('maxid');
            $position=$pos + 1;
        }
        $menu_desc="";
        ...
        $this->db->insert("menu_bottom",$data);
        redirect("menu_bottom/index");
        }
    }
    
    function update($id){
        $data['list']=$this->db->query("select * from menu_bottom where id='$id'")->row();
        $mlist=$this->db->query("select * from menu_bottom where id='$id'")->row();
        $data['menu_type']=$this->db->query("select * from menu_type order by sort asc")->result();
        $data['menu']=$this->db->query("select * from menu_bottom where menu_parent_id='0' and menu_index='0' and id not in ($mlist->id) order by id")->result();
        $data['view']='edit';
        $this->load->view('template',$data);
    }
    ...
    function update_proses(){
        ...
        $menu_parentx=  substr($menu_parent, 2,3);
        $position="";
            $cek_menuparent=$this->db->query("select * from menu_bottom where id='$id'")->row();
            if($menu_parentx==$cek_menuparent->menu_parent_id){
                $position=$cek_menuparent->menu_position;
            }else{
                $ps=$this->db->query("select max(menu_position) as maxpos from menu_bottom where menu_parent_id='$menu_parentx'")->row();
        ...
        $this->db->where("id",$id);
        $this->db->update("menu_bottom",$data);
        redirect("menu_bottom/index");
        }    
    }
    
    function naik($menuid,$menuparent,$menuposition){
        $posup=$this->db->query("select id,menu_position from menu_bottom where menu_parent_id='$menuparent' and menu_position < '$menuposition' ORDER BY menu_position desc limit 1")->row();
        $this->db->query("update menu_bottom set menu_position ='$menuposition' where id='$posup->id'");
        $this->db->query("update menu_bottom set menu_position ='$posup->menu_position' where id='$menuid'");
        redirect("menu_bottom/index");
        //echo "id".$posup->id ."- posisi".$posup->menu_position;
    }
    
    function turun($menuid,$menuparent,$menuposition){
        $posup=$this->db->query("select id,menu_position from menu_bottom where menu_parent_id='$menuparent' and menu_position > '$menuposition' ORDER BY menu_position asc limit 1")->row();
        $this->db->query("update menu_bottom set menu_position ='$menuposition' where id='$posup->id'");
        $this->db->query("update menu_bottom set menu_position ='$posup->menu_position' where id='$menuid'");
        redirect("menu_bottom/index");
        //echo "id".$posup->id ."- posisi".$posup->menu_position;
    ...
    function cek_delete(){
        $id=$this->input->post('id');
        $cek=$this->db->query("select id from menu_bottom where menu_parent_id = '$id'")->num_rows();
        if($cek > 0){
            echo "1";
        }else{
            echo "0";
    ...
    function delete($id){
        $this->db->query("delete from menu_bottom where menu_parent_id = '$id'");
        $this->db->query("delete from menu_bottom where id='$id'");
        redirect("menu_bottom/index");
    }
