<?php
class M_timeliner extends CI_Model{
        ...
        public function handler2($searchterm)
	{
            if($searchterm)
            {
                    $this->session->set_userdata('content_sr', $searchterm);
                    return $searchterm;
            }
            elseif($this->session->userdata('content_sr'))
            {
                    $searchterm = $this->session->userdata('content_sr');
                    return $searchterm;
            }
            else
            {
        ...
        public function handler3($searchterm)
	{
            if($searchterm)
            {
                    $this->session->set_userdata('year_sr', $searchterm);
                    return $searchterm;
            }
            elseif($this->session->userdata('year_sr'))
            {
                    $searchterm = $this->session->userdata('year_sr');
                    return $searchterm;
            }
            else
            {
        ...
        public function handler4($searchterm)
	{
            if($searchterm)
            {
                    $this->session->set_userdata('end_sr', $searchterm);
                    return $searchterm;
            }
            elseif($this->session->userdata('end_sr'))
            {
                    $searchterm = $this->session->userdata('end_sr');
                    return $searchterm;
            }
            else
            {
