...
<script language="javascript">
   function setModel(id,field){
     window.opener.document.forms[0].<?=$this->uri->segment(3);?>.value = id;
     window.opener.document.forms[0].<?="b".$this->uri->segment(3);?>.src = "<?=base_url();?>assets/elfinder/"+id;
     window.self.close();
	...
	$( '#container' ).on('click', 'LI A', function() {
		...
		if( entry.hasClass('folder') ) {
			...
			$( '#selected_file' ).text( "File:  " + $(this).attr( 'rel' ));
                        ...
                        var field="<?=$this->uri->segment(3);?>";
                        setModel($(this).attr( 'rel' ),field);
		}
	return false;
	});
