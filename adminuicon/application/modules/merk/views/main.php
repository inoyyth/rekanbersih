...
<div style="margin-bottom: 1%;">
    ...
    <button type="button" class="btn btn-primary disabled" style="margin-left: 2px;">Merk</button>
...
<div class="panel panel-default">
    <div class="panel-heading">Merk Management</div>
        <div class="panel-body">
            <div style="margin-bottom: 5px;">
                <input type="button" class="btn btn-primary btn-xs" value=" + New" id="new" />
                    <form method="post" action="<?=base_url();?>merk/search" id="form2"/>
                ...
                <table class="table table-bordered table-hover table-striped tablesorter">
                    ...
                    <?php
                      ...
                      foreach($data as $data){
                          $id_brand=$data->id_merk;
                          $no++;
                    ?>
                        <tr>
                            <td style="text-align: center;"><?php echo $data->id_merk; ?></td>
                            <td style="text-align: center;"><?php echo $data->name; ?></td>
                            <td style="text-align: center;"><?php echo $data->abbreviation; ?></td>
                            <td style="text-align: center;">
                                <button id="update" onclick="updatex('<?=$data->id_merk;?>');" type="button" class="btn btn-warning btn-xs"><i class="fa fa-edit"> Edit</button></i></button> 
                                <button id="delete" onclick="deletex('<?=$data->id_merk;?>');" type="button" class="btn btn-danger btn-xs"><i class="fa fa-retweet"> Delete</i></button>
                            </td>
                        </tr>
                    <?php }} ?>
                    </tbody>
                    <tfoot>
                        <tr>
                            <form method="post" action="<?=base_url();?>merk/search" id="form1"/>
...
<script type="text/javascript">
    $(document).ready(function (){
        $("#new").click(function(){
           window.location.replace("<?php echo base_url(); ?>merk/add");
        });
        });
        function updatex(id){
        window.location.replace("<?php echo base_url(); ?>merk/update/"+id+"/<?=($this->uri->segment(3)) ? $this->uri->segment(3) : 0 ;?>");
        }
         function deletex(id){
               if(confirm('Delete this record?')){
               window.location.replace("<?php echo base_url(); ?>merk/delete/"+id+"/<?=($this->uri->segment(3)) ? $this->uri->segment(3) : 0 ;?>");
                }else{
