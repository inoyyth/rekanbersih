<ol class="breadcrumb">
    <li> Coupon System</li>
    <li class="active"></i> Coupon System</li>
</ol>
<div class="panel panel-default">
    <div class="panel-heading">List Of Coupon</div>
        <div class="panel-body">
            <div style="margin-bottom: 5px;">
                <input type="button" class="btn btn-primary btn-xs" value=" + New" id="new" />
                    <form method="post" action="<?=base_url();?>coupon/search" id="form2"/>
                ...
                <table class="table table-bordered table-hover table-striped tablesorter">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align: center;">Code <i class="fa fa-sort"></i></th>
                            <th style="text-align: center;">Type <i class="fa fa-sort"></i></th>
                            <th style="text-align: center;">Expiry <i class="fa fa-sort"></i></th>
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="text-align: center;"><?php echo $data->coupon_code; ?></td>
                            <td style="text-align: center;"><?php echo $data->coupon_type; ?></td>
                            <td style="text-align: center;"><?php echo $data->expiry_date; ?></td>
                      ...
                      <?php }} ?>
                        <tr>
                            <form method="post" action="<?=base_url();?>coupon/search" id="form1"/>
                                ...
                                <td><input name="code_sr" class="form-control"value="<?=$code_sr;?>" style="width: 100%;" type="text" onkeyup="javascript:if(event.keyCode == 13){coba('form1');}else{return false;};"/></td>
                                <td><input name="type_sr" class="form-control"value="<?=$type_sr;?>" style="width: 100%;" type="text" onkeyup="javascript:if(event.keyCode == 13){coba('form1');}else{return false;};"/></td>
                                <td><input name="expiry_sr" class="form-control datepicker" value="<?=$expiry_sr;?>" style="width: 100%;" type="text" onkeyup="javascript:if(event.keyCode == 13){coba('form1');}else{return false;};"/></td>
...
<script type="text/javascript">
    $(document).ready(function (){
        $("#new").click(function(){
           window.location.replace("<?php echo base_url(); ?>coupon/add");
        });
        });
        function indexx(){
        window.location.replace("<?php echo base_url(); ?>coupon");
        }
        function updatex(id){
        window.location.replace("<?php echo base_url(); ?>coupon/update/"+id+"/<?=($this->uri->segment(3)) ? $this->uri->segment(3) : 0 ;?>");
        }
         function deletex(id){
               if(confirm('Delete this record?')){
               window.location.replace("<?php echo base_url(); ?>coupon/delete/"+id+"/<?=($this->uri->segment(3)) ? $this->uri->segment(3) : 0 ;?>");
                }else{
