...
<div style="margin-bottom: 1%;">
    ...
    <button type="button" class="btn btn-primary disabled" style="margin-left: 2px;">Brands</button>
...
<div class="panel panel-default">
    <div class="panel-heading">Brands Management</div>
        <div class="panel-body">
            <div style="margin-bottom: 5px;">
                <input type="button" class="btn btn-primary btn-xs" value=" + New" id="new" />
                <form method="post" action="<?=base_url();?>brand_management/search" id="form1"/>
                ...
                <table class="table table-bordered table-hover table-striped tablesorter">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align: center;">Child <i class="fa fa-sort"></i></th>
                            <th style="text-align: center;">Products <i class="fa fa-sort"></i></th>
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="text-align: center;"><?php echo $data->brand_child_id; ?></td>
                            <td style="text-align: center;"><?=countBC("product_general","product_brand",$data->id);?></td>
                     ...
                     <?php
                        $sqbrand=  mysql_query("select * from brand where brand_child_id='$id_brand'");
                        while($dtbrand=  mysql_fetch_array($sqbrand)){
                    ?>
                        <tr>
                            <td style="text-align: center;"><?php echo $dtbrand['id']; ?></td>
                            <td> -- -- <?php echo $dtbrand['brand_name']; ?></td>
                            <td style="text-align: center;"><?php echo status($dtbrand['status']); ?></td>
                            <td style="text-align: center;"><?php echo $dtbrand['brand_child_id']; ?></td>
                            <td style="text-align: center;"><?php echo $dtbrand['brand_product_value']; ?></td>
                            <td style="text-align: center;">
                                <button id="update" onclick="updatex('<?php echo $dtbrand['id']; ?>');" type="button" class="btn btn-warning btn-xs"><i class="fa fa-edit"> Edit</button></i></button> 
                                <button id="delete" onclick="deletex('<?php echo $dtbrand['id']; ?>');" type="button" class="btn btn-danger btn-xs"><i class="fa fa-retweet"> Delete</i></button>
                            </td>
                        </tr>
                    <?php }}} ?>
                        <tr>
                            ...
                            <td><select class="combo_search" name="status_sr" oonchange="coba('form1');"/>
                                <?php if($status_sr=="Y"){
                                        $y="selected";
                                      }else{
                                        $y="";
                            ...
                            <td><input name="child_sr" class="form-control" value="<?=$child_sr;?>" style="width: 100%;" type="text" onkeyup="javascript:if(event.keyCode == 13){coba('form1');}else{return false;};"/></td>
...
<script type="text/javascript">
    $(document).ready(function (){
        $("#new").click(function(){
           window.location.replace("<?php echo base_url(); ?>brand_management/add");
        });
        });
        function indexx(){
        window.location.replace("<?php echo base_url(); ?>brand_management");
        }
        function updatex(id){
        window.location.replace("<?php echo base_url(); ?>brand_management/update/"+id+"/<?=($this->uri->segment(3)) ? $this->uri->segment(3) : 0 ;?>");
        }
         function deletex(id){
               if(confirm('Delete this record?')){
               window.location.replace("<?php echo base_url(); ?>brand_management/delete/"+id+"/<?=($this->uri->segment(3)) ? $this->uri->segment(3) : 0 ;?>");
                }else{
