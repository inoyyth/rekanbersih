<?php
class M_type_material extends CI_Model{
	...
	{
		return $this->db->query("select * from type_material where id_type_material LIKE '%$id_sr%' and name LIKE '%$name_sr%' and abbreviation LIKE '%$status_sr%'")->num_rows();
		
	}
	
	public function search($id_sr,$name_sr,$status_sr,$limit)
	{
		return $this->db->query("select * from type_material where id_type_material LIKE '%$id_sr%' and name LIKE '%$name_sr%' and abbreviation LIKE '%$status_sr%' limit ".$limit.",2")->result();
