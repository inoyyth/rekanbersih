    ...
    <div class="panel-heading">List Discount</div>
        <div class="panel-body">
            <div style="margin-bottom: 5px;">
                ...
                <input type="button" class="btn btn-primary btn-xs" value="Main Discount" id="main_discount"/>  
                <?php if($page_status->status=="Y"){ ?>
                    <i class="fa fa-check-square-o"></i> active
                <?php }else{ ?>
                    <i class="fa fa-times"></i> not active
                <?php } ?>
                <form method="post" action="<?=base_url();?>discount/search" id="form2"/>
                ...
                <table class="table table-bordered table-hover table-striped tablesorter">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align: center;">Start Date <i class="fa fa-sort"></i></th>
                            <th style="text-align: center;">End Date <i class="fa fa-sort"></i></th>
                    ...
                    <?php
                      ...
                      foreach($data as $data){?>
                        <tr>
                            ...
                            <td style="text-align: center;"><?php echo $data->from_date; ?></td>
                            <td style="text-align: center;"><?php echo $data->to_date; ?></td>
                    ...
                    <tfoot>
                        <tr>
                            <form method="post" action="<?=base_url();?>discount/search" id="form1"/>
                                ...
                                <td><input name="from_sr" value="<?=$from_sr;?>" class="form-control datepicker" style="width: 100%;" type="text" onkeyup="javascript:if(event.keyCode == 13){coba('form1');}else{return false;};"/></td>
                                <td><input name="to_sr" value="<?=$to_sr;?>" class="form-control datepicker" style="width: 100%;" type="text" onkeyup="javascript:if(event.keyCode == 13){coba('form1');}else{return false;};"/></td>
            ...
            </div>$("#main_discount").click(function(){
            window.location.replace("<?php echo base_url(); ?>discount/index_main_dicount");
         });
        </div>
    </div>
<!--end attribute form-->
...
<script type="text/javascript">
    $(document).ready(function (){
        $("#new").click(function(){
           window.location.replace("<?php echo base_url(); ?>discount/add");
        });
        $("#main_discount").click(function(){
        ...
        function updatex(id){
        window.location.replace("<?php echo base_url(); ?>discount/update/"+id);
        }
        function indexx(){
        window.location.replace("<?php echo base_url(); ?>discount");
        }
         function deletex(id){
               if(confirm('Delete this record?')){
               window.location.replace("<?php echo base_url(); ?>discount/delete/"+id+"/<?=($this->uri->segment(3)) ? $this->uri->segment(3) : 0 ;?>");
                }else{
