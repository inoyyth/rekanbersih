...
class M_so extends CI_Model{
	...
	{
		return $this->db->query("select * from so where id_so LIKE '%$id_sr%' and name LIKE '%$name_sr%' and abbreviation LIKE '%$status_sr%'")->num_rows();
		
	}
	
	public function search($id_sr,$name_sr,$status_sr,$limit)
	{
		return $this->db->query("select * from so where id_so LIKE '%$id_sr%' and name LIKE '%$name_sr%' and abbreviation LIKE '%$status_sr%' limit ".$limit.",2")->result();
