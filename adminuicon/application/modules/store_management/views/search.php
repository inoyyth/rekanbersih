<ol class="breadcrumb">
    <li> Locator Mgt.</li>
    <li class="active"></i> Store</li>
</ol>
<div style="margin-bottom: 1%;">
    <button type="button" onclick="window.location.replace('<?php echo base_url(); ?>city_management');" class="btn btn-default" style="margin-left: 2px;">City</button>
    <button type="button" class="btn btn-primary disabled" style="margin-left: 2px;">store</button>
</div>
        ...
        <div class="panel-body">
            <div style="margin-bottom: 5px;">
                <input type="button" class="btn btn-primary btn-xs" value=" + New" id="new" />
                    <form method="post" action="<?=base_url();?>store_management/search" id="form2"/>
            ...
            <div class="table-responsive" style="font-size: 12px;">
                <table class="table table-bordered table-hover table-striped tablesorter">
                    ...
                    ?>
                      ...
                      <?php }} ?>
                        <tr>
                            <form method="post" action="<?=base_url();?>store_management/search" id="form1"/>
                                ...
                                <td><input name="city_sr" class="form-control" value="<?=$city_sr;?>" style="width: 100%;" type="text" onkeyup="javascript:if(event.keyCode == 13){coba('form1');}else{return false;};"/></td>
...
<script type="text/javascript">
    $(document).ready(function (){
        $("#new").click(function(){
           window.location.replace("<?php echo base_url(); ?>store_management/add");
        });
        });
        function viewy(id){
        ...
        function indexx(){
        window.location.replace("<?php echo base_url(); ?>store_management");
        }
        function updatex(id){
        window.location.replace("<?php echo base_url(); ?>store_management/update/"+id+"/<?=($this->uri->segment(3)) ? $this->uri->segment(3) : 0 ;?>");
        }
         function deletex(id){
               if(confirm('Delete this record?')){
               window.location.replace("<?php echo base_url(); ?>store_management/delete/"+id+"/<?=($this->uri->segment(3)) ? $this->uri->segment(3) : 0 ;?>");
                }else{
