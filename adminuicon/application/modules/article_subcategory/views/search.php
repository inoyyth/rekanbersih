<ol class="breadcrumb">
    <li> Article</li>
    <li class="active"></i> Sub Category</li>
</ol>
        ...
        <div class="panel-body">
            <div style="margin-bottom: 5px;">
                <input type="button" class="btn btn-primary btn-xs" value=" + New" id="new" />
                    <form method="post" action="<?=base_url();?>article_subcategory/search" id="form2"/>
            ...
            <div class="table-responsive" style="font-size: 12px;">
                <table class="table table-bordered table-hover table-striped tablesorter">
                    ...
                    ?>
                      ...
                      <?php }} ?>
                        <tr>
                            <form method="post" action="<?=base_url();?>article_subcategory/search" id="form1"/>
                                ...
                                <td><input name="categoryname_sr" class="form-control" value="<?=$categoryname_sr;?>" style="width: 100%;" type="text" onkeyup="javascript:if(event.keyCode == 13){coba('form1');}else{return false;};"/></td>
...
<script type="text/javascript">
    $(document).ready(function (){
        $("#new").click(function(){
           window.location.replace("<?php echo base_url(); ?>article_subcategory/add");
        });
        });
        function indexx(){
        window.location.replace("<?php echo base_url(); ?>article_subcategory");
        }
        function updatex(id){
        window.location.replace("<?php echo base_url(); ?>article_subcategory/update/"+id+"/<?=($this->uri->segment(3)) ? $this->uri->segment(3) : 0 ;?>");
        }
         function deletex(id){
               if(confirm('Delete this record?')){
               window.location.replace("<?php echo base_url(); ?>article_subcategory/delete/"+id+"/<?=($this->uri->segment(3)) ? $this->uri->segment(3) : 0 ;?>");
                }else{
