<?php
class M_unit_measure extends CI_Model{
	...
	{
		return $this->db->query("select * from merk where id_merk LIKE '%$id_sr%' and name LIKE '%$name_sr%' and abbreviation LIKE '%$status_sr%'")->num_rows();
		
	}
	
	public function search($id_sr,$name_sr,$status_sr,$limit)
	{
		return $this->db->query("select * from merk where id_merk LIKE '%$id_sr%' and name LIKE '%$name_sr%' and abbreviation LIKE '%$status_sr%' limit ".$limit.",2")->result();
